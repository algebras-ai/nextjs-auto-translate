...
class AlgebrasTranslationProvider {
    ...
    async translateBatch(texts, targetLanguage, sourceLanguage = 'en') {
        ...
        try {
            ...
            if (this.quotaExceeded || this.rateLimitExceeded) {
                const reason = this.quotaExceeded ? 'Quota' : 'Rate limit';
                console.error(`   ⚠️  ${reason} exceeded - using fallback translations for remaining items\n`);
            }
            else {
    ...
    async translateAll(textsMap, targetLanguages, sourceLanguage = 'en') {
        ...
        for (const targetLang of targetLanguages) {
            ...
            if (this.quotaExceeded || this.rateLimitExceeded) {
                const reason = this.quotaExceeded ? 'Quota' : 'Rate limit';
                console.log(`[AlgebrasTranslation] ${reason} exceeded - using fallback translations for all texts to ${targetLang}...`);
            ...
            for (let i = 0; i < texts.length; i += batchSize) {
                ...
                if (this.quotaExceeded || this.rateLimitExceeded) {
                    const reason = this.quotaExceeded ? 'Quota' : 'Rate limit';
                    console.log(`[AlgebrasTranslation] ${reason} exceeded during batch processing - using fallback translations for remaining batches...`);
                ...
                if (this.quotaExceeded || this.rateLimitExceeded) {
                    const reason = this.quotaExceeded ? 'Quota' : 'Rate limit';
                    console.log(`[AlgebrasTranslation] ${reason} exceeded detected - using fallback translations for remaining items...`);
                    // Generate fallback translations for all remaining texts in this language
