    ...
    const t = (key) => {
        ...
        if (!dictionary.files[fileKey]) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[useTranslation] File "${fileKey}" not found in dictionary`);
            }
        ...
        if (!entry) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[useTranslation] Entry "${entryKey}" not found in file "${fileKey}"`);
            }
            return key; // Return key as fallback
        }
        ...
        if (!content) {
            if (process.env.NODE_ENV === 'development') {
                console.error(`[useTranslation] Content for locale "${locale}" not found in "${key}"`);
            }
