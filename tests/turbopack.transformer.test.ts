  ...
  return (
    <html>
      <body>{children}</body>
			...
			expect(result).toContain("algebras-auto-intl/runtime/server/IntlWrapper");
			expect(result).toContain("<IntlWrapper>");
		});
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return <div>Hello</div>;
			...
			expect(result).toContain("<Translated");
			expect(result).toContain(`tKey="${relativePath}::${scopePath}"`);
			expect(result).toContain("algebras-auto-intl/runtime/client/components/Translated");
		});
  ...
  return <div>Hello</div>;
			...
			const componentRelativePath = getRelativePath(`${tempDir}/src/Component.tsx`);
  ...
  return <main><h1>Hello</h1></main>;
			...
			expect(result).toContain("algebras-auto-intl/runtime/client/components/LocaleSwitcher");
		});
