...
describe('transformProject - stability', () => {
  it('returns original code when file not in scopeMap', () => {
    ...
    const result = transformProject(code, {
      filePath: `${process.cwd()}/other.tsx`,
      sourceMap: { files: {} } as any,
    });
  ...
  it('returns original code on parse error', () => {
    ...
    const result = transformProject(code, {
      ...
      sourceMap: { files: { [file]: { scopes: {} } } } as any,
    });
    expect(result).toBe(code);
  });
  ...
  for (let i = 0; i < 19; i++) {
    it(`processes valid TSX without throwing (#${i + 1})`, () => {
      ...
      const code =
        'export default function C(){return <main><h1>Hello</h1><p>World</p></main>}';
      const result = transformProject(code, {
        filePath: `${process.cwd()}/${file}`,
        sourceMap: { files: { [file]: { scopes: {} } } } as any,
      });
